# 🚀 MicLink 快速启动指南

## ⚡ 5分钟极速版

### 前置检查
- [ ] 已安装 Go 1.21+ (`go version`)
- [ ] 已安装 JDK 17+ (`java -version`)
- [ ] 已安装 Android Studio 或配置好adb
- [ ] 准备好两台Android设备（或1台设备+1个模拟器）
- [ ] 电脑和手机在同一WiFi网络

### 第一步：启动服务器（1分钟）

**Windows：**
```powershell
# 双击运行或在PowerShell中执行
.\start-server.bat
```

**Linux/macOS：**
```bash
chmod +x start-server.sh
./start-server.sh
```

**验证服务器启动成功：**
- 浏览器访问 `http://localhost:8080/health`
- 应该看到：`{"status":"ok","online_users":0}` ✓

### 第二步：配置IP地址（30秒）

#### 获取电脑IP地址

**Windows：**
```powershell
ipconfig
# 找 "IPv4 地址"，例如: 192.168.1.100
```

**Linux/macOS：**
```bash
ifconfig
# 或 ip addr show
```

#### 修改配置文件

打开文件：
```
android/app/src/main/kotlin/com/miclink/network/Config.kt
```

修改第11行（把IP改成你的）：
```kotlin
private const val DEV_SERVER_IP = "192.168.1.100"  // ← 改成你的IP
```

**⚠️ 重要**：手机和电脑必须在同一WiFi网络！

### 第三步：编译应用（2分钟）

```powershell
cd android
.\gradlew assembleDebug
```

等待编译完成...

APK位置：`android\app\build\outputs\apk\debug\app-debug.apk`

### 第四步：安装和测试（1分钟）

**方法A：使用Android Studio（推荐）**
1. 用Android Studio打开 `android` 文件夹
2. 连接设备或启动模拟器
3. 点击 Run ▶️

**方法B：使用ADB**
```powershell
# 安装到已连接的设备
adb install android\app\build\outputs\apk\debug\app-debug.apk

# 如果提示已安装，使用-r参数重新安装
adb install -r android\app\build\outputs\apk\debug\app-debug.apk
```

### 开始通话

1. **第一台设备**
   - 打开MicLink应用
   - 输入用户ID：`user1`
   - 点击"加入"

2. **第二台设备**
   - 打开MicLink应用
   - 输入用户ID：`user2`
   - 点击"加入"

3. **发起通话**
   - 在第一台设备上看到 `user2` 在线
   - 点击 `user2` 旁边的电话图标

4. **接听通话**
   - 第二台设备看到来电
   - 点击"接听"

5. **开始说话** 🎉

---

## 📖 详细指南

### 服务器启动详解

#### 方式1：使用启动脚本（推荐）

**Windows：**
```powershell
.\start-server.bat
```

**Linux/macOS：**
```bash
./start-server.sh
```

脚本会自动：
1. 检查Go环境
2. 拉取依赖包
3. 编译服务器
4. 监听 :8080 端口

看到以下信息表示成功：
```
MicLink Signaling Server starting on :8080
```

#### 方式2：手动启动

```bash
cd server
go mod download      # 下载依赖
go run cmd/server/main.go
```

### 客户端配置详解

#### IP配置三种方式

**方式1：本机测试（推荐初学者）**
```kotlin
private const val DEV_SERVER_IP = "192.168.1.100"  // 你的电脑IP
```

**方式2：Docker容器**
```kotlin
private const val DEV_SERVER_IP = "docker.for.mac.localhost"  // macOS Docker
private const val DEV_SERVER_IP = "172.17.0.1"                // Linux Docker
```

**方式3：远程服务器**
```kotlin
private const val DEV_SERVER_IP = "example.com"  // 或 IP地址
```

#### 检查网络连接

```powershell
# 测试服务器是否可达
ping 192.168.1.100

# 测试WebSocket端口
# 在手机浏览器访问
http://192.168.1.100:8080/health
```

### 编译选项

#### 快速调试版本
```powershell
cd android
.\gradlew assembleDebug
```

#### 发布版本（需要签名）
```powershell
cd android
.\gradlew assembleRelease
```

#### 直接安装到设备
```powershell
cd android
.\gradlew installDebug
```

### 测试场景

#### 场景1：本地WiFi测试
```
设备1 ──WiFi──┬── 服务器 (PC)
             │
设备2 ────────┘
```
**配置**：所有设备使用同一WiFi，IP为PC的内网IP

#### 场景2：模拟器 + 真机
```
模拟器 ──USB─── PC ── WiFi ── 真机
```
**配置**：
- 模拟器：`DEV_SERVER_IP = "10.0.2.2"`（Android模拟器访问PC的特殊IP）
- 真机：`DEV_SERVER_IP = "PC的WiFi IP"`

#### 场景3：两个模拟器
```
模拟器1 ──┐
          PC ── :8080
模拟器2 ──┘
```
**配置**：两个模拟器都使用 `DEV_SERVER_IP = "10.0.2.2"`

---

## 🐛 常见问题

### Q1: 服务器无法启动

**症状**：`listen tcp :8080: bind: address already in use`

**原因**：8080端口已被占用

**解决**：
```powershell
# Windows - 查找占用端口的进程
netstat -ano | findstr :8080

# 结束进程
taskkill /PID <PID> /F
```

### Q2: Android客户端无法连接服务器

**症状**：显示"连接中..."或"连接失败"

**排查步骤**：
1. 检查服务器运行：浏览器访问 `http://localhost:8080/health`
2. 检查IP配置：`Config.kt` 中的 `DEV_SERVER_IP` 是否正确
3. 检查网络：手机和电脑在同一WiFi？
4. 检查防火墙：允许8080端口？

**解决**：
```powershell
# Windows - 查看IP
ipconfig

# 临时关闭防火墙测试
# 控制面板 -> Windows Defender 防火墙 -> 关闭

# 或添加防火墙规则
netsh advfirewall firewall add rule name="MicLink" dir=in action=allow protocol=TCP localport=8080
```

### Q3: 编译失败

**症状**：`Gradle sync failed` 或 `Task failed`

**原因**：依赖下载失败

**解决**：
```powershell
# 清除构建缓存
cd android
.\gradlew clean

# 重新同步
.\gradlew sync

# 重新编译
.\gradlew assembleDebug
```

### Q4: 两台设备无法通话

**症状**：发起通话后对方看不到来电

**排查**：
1. 两个设备都能看到对方ID？
2. 服务器日志有错误？
3. WebSocket连接正常？

**调试**：
```bash
# 查看服务器日志
adb logcat | findstr MicLink

# 查看WebSocket连接状态
# Chrome访问 chrome://webrtc-internals
```

### Q5: 没有声音

**症状**：通话连接成功但听不到声音

**原因**：麦克风权限或音频设备问题

**解决**：
1. 授予麦克风权限：应用 -> 权限 -> 麦克风
2. 检查音量：不要按了静音按钮
3. 切换扬声器：试试听筒/扬声器切换
4. 检查麦克风：用其他App测试麦克风是否正常

---

## 🔧 高级配置

### 自定义音质

打开 `HomeScreen.kt`，修改默认音质：
```kotlin
var audioQuality by remember { mutableStateOf(AudioQuality.MEDIUM) }
// 改为
var audioQuality by remember { mutableStateOf(AudioQuality.HIGH) }
```

### 自定义连接模式

```kotlin
var connectionMode by remember { mutableStateOf(ConnectionMode.AUTO) }
// 改为
var connectionMode by remember { mutableStateOf(ConnectionMode.P2P_ONLY) }
```

### 修改服务器端口

**Android客户端** (`Config.kt`)：
```kotlin
private const val DEV_SERVER_PORT = 8080  // 改成你的端口
```

**Go服务器** (`server/cmd/main.go`)：
```go
router.Run(":8080")  // 改成你的端口
```

### 启用详细日志

**Android客户端** (`Config.kt`)：
```kotlin
const val DEBUG_ENABLED = true  // 启用详细日志
```

**查看日志**：
```powershell
adb logcat | findstr MicLink
```

---

## ✅ 测试检查清单

### 功能测试
- [ ] 服务器成功启动
- [ ] APK成功编译和安装
- [ ] App启动时请求麦克风权限
- [ ] 可以输入用户ID并加入
- [ ] 可以看到在线用户列表
- [ ] 可以发起通话
- [ ] 对方收到来电提示
- [ ] 可以接听通话
- [ ] 通话中有清晰的声音
- [ ] 静音按钮正常工作
- [ ] 扬声器切换正常
- [ ] 挂断按钮正常工作

### 网络测试
- [ ] WiFi连接正常
- [ ] P2P直连成功
- [ ] 自动降级到中转
- [ ] 网络切换自动恢复

### 资源测试
- [ ] 内存占用 < 100MB
- [ ] CPU占用 < 10%（通话中）
- [ ] 电池消耗正常
- [ ] 不会过度发热

---

## 📞 获取帮助

- **文档**：查看 [docs/](.) 目录下的其他文档
- **问题**：查看 [TESTING.md](TESTING.md) 的故障排查部分
- **深度技术**：查看 [ARCHITECTURE.md](ARCHITECTURE.md)
- **生产部署**：查看 [DEPLOYMENT.md](DEPLOYMENT.md)

---

## 🎉 恭喜！

你现在已经成功运行了MicLink！

**下一步**：
1. 读一下 [ARCHITECTURE.md](ARCHITECTURE.md) 了解系统设计
2. 查看 [API.md](API.md) 理解通信协议
3. 根据需要修改 [DEPLOYMENT.md](DEPLOYMENT.md) 中的配置进行生产部署

**祝你使用愉快！** 🚀
